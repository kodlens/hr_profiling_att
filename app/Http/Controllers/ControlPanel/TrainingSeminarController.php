...
class TrainingSeminarController extends Controller
{
    ...
    public function create(){
        return view('control-panel.training_seminar.training-seminar-create-edit')
            ->with('id', 0)
            ->with('data', []);
    }
    ...
    public function updateTrainingDSeminar(Request $req, $id){
        ...
        if($req->hasFile('file')) {
            ...
            if(Storage::exists('public/trainings/' .$data->attach_path)) {
                Storage::delete('public/trainings/' . $data->attach_path);
            }
        }
